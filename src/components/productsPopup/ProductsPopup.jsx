...
const ProductsPopup = ({toggleProductPopup,productPopup,id}) => {
  ...
  return (
    <div className={`w-screen h-screen bg-black bg-opacity-95 fixed top-0 left-0 z-50 flex justify-center items-center md:items-center transform transition-transform duration-300 ${productPopup ? 'translate-y-0' : '-translate-y-full'}`}>
            {
                productPopup &&
                <div className={`w-[100%] h-[85%] absolute md:flex md:h-[80%] md:w-[90%] bg-gray-700 rounded-t-[40px] md:rounded-3xl flex flex-col justify-center items-center bg-popupProducts bg-cover bg-top shadow-md shadow-orange-900/50`}>
                    ...
                    <div className="text-white p-8 h-full">
                        <h2 className="text-xl md:text-3xl text-center font-bold">{t(`productPopupTitle${id}`)}</h2>
                        <ul className="mt-8 list-disc list-outside	gap-2 flex flex-col text-sm md:text-2xl justify-center h-full">
                    ...
                    <button onClick={() => navigateContactMd(id)} className="bg-[#FB432C] text-white font-semibold text-[16px] md:text-[26px] rounded-2xl px-[20px] py-[10px] m-6 hover:text-[#FB432C] hover:bg-white hover:scale-110">
                        {t('navBarContact')}
                    </button>
                </div>
            }
